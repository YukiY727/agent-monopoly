...
**テストが落ちたときは、原因と解決策を詳細に記録する**

テスト失敗は学びの機会。なぜ失敗したのか、どう解決したのかを明確にすることで、同じ問題の再発を防ぎ、設計の改善につながる。
...
2. **失敗原因を分類する**
   - **実装バグ**: ロジックの誤り、境界条件の見落とし
   - **テストの不備**: Given-When-Thenの不明確さ、期待値の誤り
   - **設計の問題**: 責任の所在が不明確、依存関係の複雑さ
...
- **原因**: なぜテストが失敗したのか（技術的詳細）
- **解決**: どう修正したのか（変更内容）
- **学び**: どの原則を見落としていたのか、今後の予防策

例:
```
fix: Playerの残高がマイナスになる不具合を修正

原因: Property.collectRent()がプレイヤー残高の事前チェックを
      していなかった。防御的プログラミングの欠如。

解決: require(player.balance >= rent)を追加し、
      残高不足時はInsufficientFundsExceptionをスロー

学び: 金銭操作には必ず事前条件チェックを。
      防御的プログラミングの原則を徹底する。
```
