...
コードにバグがあったら：
1. どのテストケースが足りていないかを考える
2. そのテストケースを書く（失敗することを確認）
3. アプリケーションのコードを修正する

バグはテストの不足を示すシグナル。修正前にテストで再現することで、再発を防ぐ。

**複雑なシナリオのテスト**
- 複雑なシナリオが必要な場合、その複雑なシナリオを全て漏らさずテストに書く
- エッジケース、境界値、例外パスを網羅的にテストする
- 複雑さを避けるのではなく、テストで明示的に記述することで仕様を明確化する
