...
**このドキュメントのコード例は「参考例」であり、「こう書かなければならない」という強制ではありません。**

### TDDでの柔軟なコーディング

このドキュメントには多数のコード例が記載されていますが：

- ✅ **参考例として活用**する
- ✅ **実装中により良い方法が見つかればそちらを採用**する
- ✅ **テストを書きながら、最適なコードを見つける**
- ❌ **この通りに書かなければならない**わけではない
...
// 実装中の判断: EventRecorderクラスに分離した方が良いと判断
eventRecorder.record(gameState, PropertyPurchased(...))

// → より良い設計が見つかれば、ドキュメントの例を無視してOK
```
