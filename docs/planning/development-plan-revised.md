...
**作成日**: 2025-11-24  
**改訂理由**: ゲームロジックの完全実装を優先し、イベント記録・HTMLレポート機能を後回しにする
...
- Phase 2でイベント記録システム（EventLogger、JSON保存、HTMLレポート）を実装
- その後Phase 3-7でゲームロジック（建物、ダブル、刑務所、カード、税金、抵当、オークション）を追加
- **問題**: ゲームルールを追加するたびにイベント記録とレポート機能を修正する必要がある

### 新計画のアプローチ

1. **Phase 2-7**: ゲームロジックを完全に実装
2. **Phase 8-9**: 完全なゲームロジックに対してイベント記録とHTMLレポートを実装
3. **Phase 10以降**: シミュレーション機能と統計分析

**利点**:
- イベント記録の構造が安定してから永続化機能を実装
- レポート機能の修正回数を削減
- 完全なゲームルールに対してテストを書ける
...
#### 実装範囲
- 最小限のデータ構造（ボード、プレイヤー、プロパティ）
- 基本ゲームロジック（サイコロ、移動、購入、破産、ゲーム終了）
- CLI出力（勝者の表示のみ）
...
**目標**: 異なる戦略を実装し、戦略の比較を可能にする

#### 開発ステップ

1. **詳細仕様**: 戦略インターフェースの拡張、各戦略の仕様
2. **設計**: 追加戦略の設計
3. **実装**: 複数戦略の実装
4. **テスト**: 各戦略の動作確認
...
**目標**: ゲーム進行を最初から最後まで追跡できるようにする（完全実装）
...
- **イベント記録機能**
  - GameEventのsealed class定義（全イベントタイプ）
  - GameStateへのイベントログ追加
  - GameServiceでのイベント記録
  - ターンごとの状態記録
  - 重要なイベント（購入、破産、建物、カード、税金、抵当、オークションなど）の記録
...
**目標**: ゲーム結果をHTMLレポートで視覚化する

#### 開発ステップ

1. **詳細仕様**: HTMLレポートの仕様、表示項目の定義
2. **設計**: HtmlReportGeneratorの設計
3. **実装**: HTMLレポート生成機能
4. **テスト**: ブラウザで表示確認
...
| Phase 2 | 3-5日 | 建物システムとダブル |
| Phase 3 | 5-7日 | 刑務所とカードシステム |
| Phase 4 | 2-3日 | 税金と特殊プロパティ |
| Phase 5 | 2-3日 | 抵当システム |
| Phase 6 | 3-5日 | オークションシステム |
| Phase 7 | 3-5日 | 複数戦略の実装 |
...
| フェーズ | 期間（想定） | 主要成果物 |
|---------|------------|------------|
| Phase 8 | 3-5日 | イベント記録システム |
| Phase 9 | 3-5日 | HTMLレポート生成 |

**イベント記録とレポート完成**: 約1-2週間

### シミュレーション基盤
| フェーズ | 期間（想定） | 主要成果物 |
|---------|------------|------------|
| Phase 10 | 2-3日 | 複数ゲーム実行と基本統計 |
| Phase 11 | 3-5日 | 統計の可視化 |
| Phase 12 | 5-7日 | 並列実行 |
...
| フェーズ | 期間（想定） | 主要成果物 |
|---------|------------|------------|
| Phase 13 | 1-2週間 | 詳細統計と高度な分析 |
| Phase 14 | 2-4週間 | Web UI（オプション） |

**全機能完了**: 約2-3ヶ月（Phase 14を含む場合）
...
- **実測に基づく**: ベンチマーク、プロファイリング結果で判断する（Phase 12のパフォーマンス評価など）
...
**Phase 1-7での方針:**
- **イベントログの記録**: ゲーム内の全イベントを `List<GameEvent>` でメモリ内に保持
- **シンプルな実装**: JSON保存やHTMLレポートは実装しない
- **Phase 8で完全実装**: ゲームロジックが完成してから、完全なイベント記録システムを実装
...
| イベント記録の構造変更 | Phase 8でのリファクタリング | Phase 1-7でイベント型を定義し、Phase 8で永続化のみ追加 |
