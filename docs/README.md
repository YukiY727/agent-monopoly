...
このプロジェクトでは「**全てのアーキテクチャはトレードオフである**」という原則を重視しています。

- 完璧なアーキテクチャは存在しない。何かを得る代わりに何かを失う
- トレードオフを認識し、プロジェクトの価値観（保守性 > パフォーマンス > 開発速度）に基づいて意思決定
- YAGNI原則、実測に基づく判断、段階的な投資を重視

詳細は [claude.md](../claude.md) の「アーキテクチャのトレードオフ原則」と [技術選定](planning/technology-selection.md) を参照してください。
